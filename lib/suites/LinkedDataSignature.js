...
module.exports = class LinkedDataSignature extends LinkedDataProof {
  ...
  async getVerificationMethod({proof, documentLoader}) {
    ...
    // console.error('verificationMethod', verificationMethod, {
    //   '@context': constants.SECURITY_CONTEXT_URL,
    //   '@embed': '@always',
    //   id: verificationMethod
    // }, {documentLoader, compactToRelative: false, omitGraph: true});

    const framedResults = await jsonld.frame(verificationMethod, {
      '@context': constants.SECURITY_CONTEXT_URL,
      '@embed': '@always',
      id: verificationMethod
    }, {documentLoader, compactToRelative: false, omitGraph: false});

    // console.error('framedResults', framedResults)
    const framed = framedResults['@graph'] ? framedResults['@graph'][0] : (framedResults || null);

    // console.error('framed', framed)

    // const {'@graph': [framed]} = framedResults;
    if(!framed) {
